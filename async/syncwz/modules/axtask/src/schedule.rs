...
pub(crate) fn notify_wait_for_exit(task: &AxTaskRef, rq: &mut AxRunQueue) {
    if let Some(wait_queue) = WAIT_FOR_TASK_EXITS.lock().remove(&task.tid()) {
