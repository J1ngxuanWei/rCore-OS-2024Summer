...
use crate::executor::spawn_user_task;
...
impl Task {
    ...
    pub fn set_old_tf(&self, tf: TrapFrame) {
        self.tf.lock().save_old(tf)
    }
...
impl Task {
    ...
    pub fn new(
        ...
        Self {
            ...
            //TODOWJX:优先级应该一样，但是为了演示，改成10+id
            pri: 10 + iid,
        }
    }
    ...
    pub fn init(args: Vec<String>, envs: &Vec<String>) -> AxResult<AxTaskRef> {
        ...
        spawn_user_task(Arc::clone(&axtask.clone()));
        Ok(axtask.clone())
    }
}
