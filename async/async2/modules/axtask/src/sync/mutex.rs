...
impl<T: ?Sized> Mutex<T> {
            ...
            match self.owner_id.compare_exchange_weak(
                ...
                Err(owner_id) => {
                    assert_ne!(
                        ...
                        current().inner.lock().id_name()
                    );
                    axlog::warn!("lock fail");
    ...
    pub fn try_lock(&self) -> Option<MutexGuard<T>> {
        let current_id = current().inner.lock().id().as_u64();
