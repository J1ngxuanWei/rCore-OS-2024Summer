    ...
    if let Some(old_trap_frame) = signal_module.last_trap_frame_for_signal.take() {
        unsafe {
            ...
            let mut now_trap_frame =
                read_trapframe_from_kstack(current_task.get_kernel_stack_top());
            ...
            write_trapframe_to_kstack(current_task.get_kernel_stack_top(), &now_trap_frame);
        }
        true
    } else {
        false
...
pub fn signal_return() -> isize {
    if load_trap_for_signal() {
        ...
        read_trapframe_from_kstack(current_task().get_kernel_stack_top()).get_ret_code() as isize
    } else {
