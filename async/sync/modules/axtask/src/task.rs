...
use axhal::arch::{write_page_table_root0, write_trapframe_to_kstack, TrapFrame};
...
impl Task {
    ...
    pub fn get_kernel_stack_top(&self) -> usize {
        self.inner.lock().get_kernel_stack_top().unwrap()
    }
...
impl Task {
    ...
    pub fn init(args: Vec<String>, envs: &Vec<String>) -> AxResult<AxTaskRef> {
        ...
        let kernel_top = new_task.get_kernel_stack_top();
        // 将其作为内核进程的子进程
        ...
        write_trapframe_to_kstack(kernel_top, &new_trap_frame);
...
extern "C" fn task_entry() -> ! {
    ...
    if let Some(entry) = task.get_entry() {
        if task.is_kernel_task() {
            ...
            // 需要通过切换特权级进入到对应的应用程序
            let kernel_sp = task.get_kernel_stack_top();
            // 切换页表已经在switch实现了
            // 记得更新时间
            task.inner
                .lock()
